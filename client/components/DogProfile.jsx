...
class DogProfile extends React.Component {
  ...
  componentDidMount() {
    ...
    getDog(this.props.match.params.id).then(dog => {
      getOwner(dog.owner_id).then(owner =>
        this.setState({
  ...
  handleWalk(e) {
    ...
    getOwner(this.state.owner_id).then(owner => {
      this.setState({
  ...
  render() {
    return (
      <div className="dogprofiledisplay">
        ...
        >
          I would like to walk this dog
        </button>
        {this.state.walk_the_dog && (
          <p>
            You would like to walk this dog. Click the request button to contact
            the owner
          </p>
        )}
